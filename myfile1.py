...
def load_documents_from_json(cube_id: str, doc_type: str, base_dir: str) -> List[Document]:
    ...
    try:
        ...
        if not os.path.exists(file_path):
            raise FileNotFoundError(f"Cube data doesn't exits")
    ...
    def get_dimensions(self, query: str, cube_id: str, prev_conv: dict) -> str:
        ...
        try:
            ...
            with get_openai_callback() as dim_cb:
                ...
                data = {"prev_query":prev_q,"prev_measures":prev_d}
    ...
    def get_measures(self, query: str, cube_id: str, prev_conv: dict) -> str:
        ...
        try:
            ...
            print(Fore.RED + '    Identifying Measure group name and level name......................\n')
                ...
                # )
            # Load BM25 documents from pickle file
            """bm25_file = os.path.join(vector_db_path, cube_id, f"{cube_id}_measures.pkl") 
            try: 
                with open(bm25_file, 'rb') as f: 
                    documents = pickle.load(f) 
                logging.info(f"Successfully loaded {len(documents)} BM25 documents for cube {cube_id}") 
            except FileNotFoundError: 
                logging.error(f"BM25 file not found at {bm25_file}") 
                raise FileNotFoundError(f"BM25 document file not found for cube {cube_id}") 
            except Exception as e: 
                logging.error(f"Error loading BM25 file: {str(e)}") 
                raise ValueError(f"Failed to load BM25 documents: {str(e)}")
            """
            # Load documents from JSON
...
class FinalQueryGenerator(LLMConfigure):
    ...
    def load_sample_queries(self, cube_id: str) -> list:
        """Load sample queries for this cube if available"""
        try:
            sample_queries_dir = os.path.join(BASE_DIR, "sample_queries")
            sample_file = os.path.join(sample_queries_dir, cube_id, f"{cube_id}_sample_queries.json")
            ...
            logging.error(f"Error loading sample queries: {e}")
            return []
    ...
    def generate_query(self, query: str, dimensions: str, measures: str, prev_conv: dict, cube_name: str) -> str:
        ...
        try:
            ...
            # In generate_query method, add:
            sample_queries = self.load_sample_queries(cube_id=cube_name)
                        
            # Prepare sample query examples
            ...
            user query:-Please provide region wise industry wise count of customers for EWS Tag HIGH for the Day 15 Jan 2025.
            Expected Response:-select [Customer Details].[Region] as "Region", [Customer Details].[Industry] as "Industry", [Customer Details].[Count of Customers] as "Count of Customers", [Customer Details].[Business Segment] as "Business Segment", [Customer Details].[EWS Tag] as "EWS Tag", [Assessment Details].[Customer EWS Score] as "Customer EWS Score" from [Cube].[{cube_name}] where [Customer Details].[EWS Tag] = 'HIGH' and [Time].[Day] = '2025-01-15'
            ...
            user query:-Which Business segments are having more than 1 customers
            Expected Response:-select [Customer Details].[Business Segment] as "Business Segment", [Customer Details].[Count of Customers] as "Count of Customers" from [Cube].[{cube_name}] where [Customer Details].[Count of Customers] > 1.00
...
class SampleQuery(BaseModel):
    # Using this to allow arbitrary key-value pairs
    __root__: Dict[str, str]
    
    @property
    def user_query(self) -> str:
        # Return the key (first and only key)
        return next(iter(self.__root__.keys()))
    
    @property
    def cube_query(self) -> str:
        # Return the value (first and only value)
        return next(iter(self.__root__.values()))
...
class CubeDetailsRequest(BaseModel):
    ...
    cube_id: str
    sample_queries: Optional[List[Dict[str, str]]] = None
...
def save_sample_queries(sample_queries: List[Dict[str, str]], cube_id: str) -> None:
    """Save sample queries to a JSON file"""
    try:
        # Create directory if it doesn't exist
        cube_dir = os.path.join(SAMPLE_QUERIES_DIR, cube_id)
        os.makedirs(cube_dir, exist_ok=True)
        
        # Format the sample queries for storage
        formatted_queries = []
        for query_dict in sample_queries:
            # Each dict should have exactly one key-value pair
            if len(query_dict) == 1:
                user_query = next(iter(query_dict.keys()))
                cube_query = next(iter(query_dict.values()))
        ...
        # Save to JSON file
        file_path = os.path.join(cube_dir, f"{cube_id}_sample_queries.json")
        with open(file_path, 'w') as f:
            json.dump({"sample_queries": formatted_queries}, f, indent=2)
    except Exception as e:
        logging.error(f"Error saving sample queries: {str(e)}")
        raise
...
async def process_cube_details(cube_json_dim, cube_json_msr, cube_id: str, sample_queries=None) -> Dict:
    try:
        # Define paths
        cube_dir = os.path.join(vector_db_path, cube_id)
           ...
           os.path.exists(os.path.join(cube_dir, f"{cube_id}_measures.json")):
            ...
            if sample_queries and len(sample_queries) > 0:
                ...
                for query in sample_queries:
                    sample_query_objects.append(SampleQuery(
                        user_query=query.user_query,
                        cube_query=query.cube_query
                    ))
                save_sample_queries(sample_queries, cube_id)
